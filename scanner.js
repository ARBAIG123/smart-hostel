...
import { collection, query, where, getDocs, doc, getDoc } from "https://www.gstatic.com/firebasejs/12.12.1/firebase-firestore.js";
...
function getNearestSlot() {
  ...
  slotMinutes.forEach((mins, i) => {
    ...
    if (diff >= -30 && Math.abs(diff) < minDiff) { minDiff = Math.abs(diff); nearest = i; }
  });
...
onAuthStateChanged(auth, async (user) => {
  ...
  html5QrCode.start(
    ...
    async (decodedText) => {
      ...
      await processQR(decodedText.trim(), currentSlot);
    },
    () => {}
  );
});

async function processQR(raw, slot) {
  ...
  try {
    // QR contains JSON with roll number
    let rollNumber = raw;
    try {
      const parsed = JSON.parse(raw);
      rollNumber = parsed.roll || raw;
    } catch(_) {}
    ...
    if (snap.empty) {
      ...
      resultSlot.style.color = 'var(--red)';
    } else {
      ...
      if (!bookedSlot) {
        ...
        resultSlot.style.color = 'var(--amber)';
      } else {
        resultIcon.textContent = '✅';
        resultName.textContent = student.name;
        resultRoll.textContent = `Roll: ${student.roll}`;
        resultSlot.textContent = `Booked slot: ${bookedSlot}`;
        resultSlot.style.color = 'var(--green)';
      }
    }
